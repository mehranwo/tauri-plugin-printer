...
// use tauri::core::process::Command;
/**
...
pub fn get_printers() -> String {
    ...
    // Spawn a new thread
    thread::spawn(move || {
        let output: tauri::core::process::Output = Command::new("powershell").args(["Get-Printer | Select-Object Name, DriverName, JobCount, PrintProcessor, PortName, ShareName, ComputerName, PrinterStatus, Shared, Type, Priority | ConvertTo-Json"]).output().unwrap();
...
pub fn print_pdf(options: PrintOptions) -> String {
    ...
    // Spawn a new thread
    thread::spawn(move || {
        let output: tauri::core::process::Output = Command::new("powershell")
            .args([shell_command])
            .output()
            .unwrap();
